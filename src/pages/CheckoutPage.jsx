          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
            ...
            <div className="lg:col-span-2">
                  ...
                  <div>
                    ...
                    <div className="space-y-4">
                      <div>
                        ...
                        <input name="address" value={form.address} onChange={handleChange} required className="w-full bg-surface border border-white/10 text-cream text-[0.85rem] px-4 py-3 focus:outline-none focus:border-lime/40" />
                      </div>
                      <div className="grid grid-cols-2 gap-4">
                        <div>
                          <label className="block text-[0.65rem] tracking-[0.2em] uppercase text-muted mb-2">City</label>
                          <input name="city" value={form.city} onChange={handleChange} required className="w-full bg-surface border border-white/10 text-cream text-[0.85rem] px-4 py-3 focus:outline-none focus:border-lime/40" />
                        </div>
                        <div>
                          <label className="block text-[0.65rem] tracking-[0.2em] uppercase text-muted mb-2">LGA</label>
                          <input name="lga" value={form.lga} onChange={handleChange} className="w-full bg-surface border border-white/10 text-cream text-[0.85rem] px-4 py-3 focus:outline-none focus:border-lime/40" />
                        </div>
                      </div>
                      <div>
                        <label className="block text-[0.65rem] tracking-[0.2em] uppercase text-muted mb-2">State</label>
                        <select name="state" value={form.state} onChange={handleChange} required className="w-full bg-surface border border-white/10 text-cream text-[0.85rem] px-4 py-3 focus:outline-none focus:border-lime/40 appearance-none">
              ...
              {step === 1 && (
                <div className="space-y-8">
                  <div className="bg-surface border border-white/[0.06] p-6">
                    <h2 className="text-cream font-semibold text-[0.82rem] tracking-[0.25em] uppercase mb-5">Bank Transfer Details</h2>
                    <div className="space-y-4">
                      {[
                        ...
                        <div key={row.label} className="flex items-center justify-between py-3 border-b border-white/[0.05]">
                          ...
                          <span className={`text-[0.9rem] font-semibold ${row.label === 'Amount to Pay' ? 'text-lime' : 'text-cream'}`}>{row.value}</span>
                        </div>
                      ))}
                    </div>
                  ...
                  {/* Upload receipt */}
                  <div className="bg-surface border border-white/[0.06] p-6">
                    <h2 className="text-cream font-semibold text-[0.82rem] tracking-[0.25em] uppercase mb-4">I've Made Payment — Upload Receipt</h2>
                    <label className={`flex flex-col items-center justify-center h-36 border-2 border-dashed cursor-pointer transition-all ${receipt ? 'border-lime/50 bg-lime/5' : 'border-white/10 hover:border-white/20'}`}>
                      ...
                      {receipt ? (
                        ...
                        <div className="text-center">
                          ...
                          <p className="text-cream/50 text-[0.78rem]">Upload payment receipt</p>
              ...
              {step === 2 && (
                <div className="space-y-6">
                  <div className="bg-surface border border-white/[0.06] p-6">
                    <h2 className="text-cream font-semibold text-[0.82rem] tracking-[0.25em] uppercase mb-5">Confirm Your Order</h2>
                    <div className="space-y-3 text-[0.85rem]">
                      {[
                        ...
                        <div key={row.label} className="flex justify-between py-2 border-b border-white/[0.04]">
                          ...
                          <span className="text-cream text-right max-w-[250px]">{row.value}</span>
                        </div>
                      ))}
                    </div>
                  </div>
                  ...
                  <div className="bg-surface border border-white/[0.06] p-6">
                    <h3 className="text-cream font-semibold text-[0.82rem] tracking-[0.25em] uppercase mb-4">Order Items</h3>
                    {cart.map(item => (
                      <div key={item.key} className="flex justify-between items-center py-3 border-b border-white/[0.04] text-[0.85rem]">
                        <span className="text-cream/80">{item.product.name} × {item.qty} <span className="text-muted">({item.size})</span></span>
                        <span className="text-cream">£{(item.price * item.qty).toFixed(2)}</span>
                      </div>
                    ))}
                    <div className="flex justify-between pt-4">
                      <span className="text-cream font-semibold text-[0.9rem]">Total</span>
                      <span className="text-lime font-bold text-[1rem]">£{total.toFixed(2)}</span>
                    </div>
                  </div>
            ...
            <div className="lg:col-span-1">
              <div className="bg-surface border border-white/[0.06] p-6 sticky top-28">
                <h3 className="text-cream font-semibold text-[0.8rem] tracking-[0.25em] uppercase mb-5">Your Bag ({cart.length})</h3>
                  ...
                  {cart.map(item => (
                    <div key={item.key} className="flex gap-3">
                      ...
                      <div className="flex-1">
                        <p className="text-cream text-[0.78rem] font-medium leading-tight mb-1">{item.product.name}</p>
                        <p className="text-muted text-[0.65rem]">{item.size} · {item.color}</p>
                        <p className="text-cream/80 text-[0.78rem] mt-1">£{item.price} × {item.qty}</p>
                      </div>
                    </div>
                  ))}
                </div>
                <div className="border-t border-white/[0.05] pt-4 space-y-2">
                  ...
                  <div className="flex justify-between text-[0.8rem]">
                    ...
                    <span className={shipping === 0 ? 'text-lime' : 'text-cream'}>{shipping === 0 ? 'FREE' : `£${shipping.toFixed(2)}`}</span>
                  </div>
                  <div className="flex justify-between pt-3 border-t border-white/[0.05]">
                    <span className="text-cream font-semibold">Total</span>
                    <span className="text-lime font-bold">£{total.toFixed(2)}</span>
                  </div>
                </div>
              </div>
            </div>
