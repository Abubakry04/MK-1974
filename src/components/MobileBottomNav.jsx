...
export default function MobileBottomNav() {
  ...
  return (
    <div className="md:hidden fixed bottom-4 inset-x-4 z-[90] pointer-events-auto">
      <div className="bg-white/95 backdrop-blur-xl border border-black/15 rounded-full px-4 py-2.5 shadow-2xl flex items-center justify-around">
        {navItems.map((item) => {
            ...
            <div className="relative flex flex-col items-center gap-1 group">
              <div
                className={`p-1.5 rounded-full transition-all duration-300 ${
                  isActive ? 'text-dark scale-110' : 'text-dark/50 group-active:scale-90 group-active:text-dark'
                }`}
              >
                {item.icon(isActive)}
              </div>
              <span
                className={`text-[0.52rem] font-bold tracking-[0.2em] uppercase transition-colors ${
                  isActive ? 'text-dark font-extrabold' : 'text-dark/50'
                }`}
              >
                {item.label}
              </span>
              ...
              {/* Badge */}
              {item.badge > 0 && (
                <span className="absolute -top-1 -right-1 bg-dark text-cream font-black text-[0.45rem] min-w-[15px] h-[15px] px-1 rounded-full flex items-center justify-center animate-bounce shadow-md">
                  {item.badge}
                </span>
              )}
            </div>
